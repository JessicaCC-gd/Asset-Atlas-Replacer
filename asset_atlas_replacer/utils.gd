...
func find_dups(files : PackedStringArray, data : Array[Resource]):
	console_print("Searching for duplicate file names")
	var file_names = []
	for f in files:
	...
	while true:
		...
		if amount > 1:
			console_print("More than one file with the same name: \"%s\". Files were not replaced" % file_names[n],  Color.DARK_RED)
		...
		if n >= file_names.size():
			break
	console_print("Searching for duplicate file names completed")
	return [files, data]
